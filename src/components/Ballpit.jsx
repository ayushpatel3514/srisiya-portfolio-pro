...
const X = {
  count: 120,
  colors: [0x111827, 0x8b5cf6, 0xffffff], // tweak: dark + purple + white
...
function createBallpit(e, t = {}) {
  ...
  const hState = S({
    domElement: e,
    onMove() {
      n.setFromCamera(hState.nPosition, i.camera);
  ...
  function initialize(eConfig) {
    if (s) {
      i.clear();
      i.scene.remove(s);
    }
    s = new Z(i.renderer, eConfig);
    i.scene.add(s);
  }

  i.onBeforeRender = (time) => {
    if (!c) s.update(time);
  };
...
const Ballpit = ({ className = "", followCursor = true, ...props }) => {
  ...
  <canvas
    ...
    style={{
      ...
      height: "100%",
      display: "block",
      pointerEvents: "none", // background only, doesn't eat scroll
    }}
  />
);
